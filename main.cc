...
namespace methods
{
    void print_timer_deltas(uint64_t START_TIME, std::vector<uint64_t> yield_at)
    {
...
namespace tests
{
    void simple_epoch(uint64_t _us_to_run)
    {
        ...
        methods::print_timer_deltas(START_TIME, yield_at);
        log::print("[Test 1]: Expected: %d , got: %d\n", 2, _us_to_run, END_TIME - START_TIME - estimate_gettime_latency());
    }

    void simple_modulo(uint64_t _us_to_run)
    {
        uint64_t _ns_to_run = _us_to_run * 1000, i = 0;
    ...
    void modulo_with_clock_speed(uint64_t _us_to_run)
    {
        std::vector<uint64_t> yield_at = {};
        uint64_t yield_cost_cc = 400, i = 0;

        // First estimate clock speed
    ...
    {
        uint64_t CLOCK_TIMESTAMP = 0, i = 0, yield_cost_cc = 200;
        std::vector<uint64_t> yield_at = {};

        float estimated_clock_speed = estimate_clock_speed();
        float clock_cycle_per_iter = methods::calc_function_cc();
...
int main()
{
    setup::rdtscp_latency();

    for (auto ms : RUN_FOR_US)
    {
        for (size_t i = 0; i < SAMPLE_SIZE; i++)
        {
            tests::simple_epoch(ms);
        }
    }
}